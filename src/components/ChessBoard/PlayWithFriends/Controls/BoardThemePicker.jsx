...
const BoardThemePicker = () => {
    ...
    return (
        <>
            <button type='button' onMouseEnter={handleOpenPicker} onMouseLeave={handleClosePicker} className="relative w-full h-full inline-flex items-center justify-center overflow-hidden text-sm font-medium  group">
                <span className="relative px-5 py-2.5 transition-all ease-in duration-75 text-gray-700 dark:text-gray-100 rounded-md group-hover:bg-opacity-0">
                    Change Theme
                    <i className="ml-2 fa-solid fa-palette"></i>
                </span>
            </button>
            {
                openPicker &&
                <div data-popover role="tooltip" onMouseEnter={handleOpenPicker} onMouseLeave={handleClosePicker} className="absolute z-10 inline-block w-32 text-sm text-gray-500 transition-opacity duration-300 bg-white border border-gray-200 rounded-lg shadow-sm dark:text-gray-400 dark:border-gray-600 dark:bg-gray-800">
